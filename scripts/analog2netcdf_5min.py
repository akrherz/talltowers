"""Create a monthly netcdf file with 5 minute windowed stats"""
...
ANALOG_VARS = ['ws_5m_s', 'ws_5m_nw', 'winddir_5m_s', 'winddir_5m_nw',
...
def create_netcdf(valid):
    ...
    timelen = (nextmonth - valid).days * 12 * 24
    nc = netCDF4.Dataset(valid.strftime("analog5min_%Y%m.nc"), 'w')
    ...
    for vname in ANALOG_VARS:
        for stat in ['mean', 'median', 'minimum', 'maximum',
                     'standard_deviation', 'median_abs_deviation', 'count']:
            _vname = "%s_%s" % (vname, stat)
            ncvar = nc.createVariable(_vname, np.double, ('station', 'time'),
                                      fill_value=1e37)
            units = "m/s"
...
def write_analog_data(valid, nc):
    ...
    medsql = ["median(%(v)s::numeric) as %(v)s_median" % dict(v=xlate(v)) for v in ANALOG_VARS]
    medsql = ", ".join(medsql)
    lgsql = [
        ("avg(%(v)s) as %(v)s_mean,\n"
         ...
         "median(abs(%(v)s - %(v)s_median)::numeric) as %(v)s_median_abs_deviation,\n"
         "sum(case when %(v)s is not null then 1 else 0 end) as %(v)s_count\n"
         ) % dict(v=xlate(v)) for v in ANALOG_VARS]
    lgsql = ", ".join(lgsql)

    cursor = PGCONN.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    for i, tstep in enumerate(tqdm(nc.variables['time'][:])):
        ets = valid + datetime.timedelta(seconds=tstep)
        sts = ets - datetime.timedelta(seconds=300)
        cursor.execute("""
        WITH meds as (
            SELECT tower, """ + medsql + """
            from data_analog_""" + table + """
            where valid >= %s and valid < %s and tower in (0, 1)
            GROUP by tower
        )
        SELECT a.tower,
            """ + lgsql + """
            from data_analog_""" + table + """ a JOIN meds m on (a.tower = m.tower)
        ...
        for row in cursor:
            for col in row:
                if col in ['tower', 'valid', 'delta']:
                    continue
                nc.variables[
                    col.replace("nwht", "nw")][row['tower'], i] = row[col]
