...
def main(argv):
    ...
    basets = datetime.datetime.strptime(nc.variables['time'].units.split()[2],
                                        '%Y-%m-%d')
    tindex = []
    for offset in nc.variables['time'][:]:
        tindex.append(basets + datetime.timedelta(seconds=offset))
    dfs = [pd.DataFrame(index=pd.Series(tindex, name='time')),
    ...
    for sid in range(nc.dimensions['station'].size):
        for vname in nc.variables:
            ...
            if ncvar.shape != (nc.dimensions['station'].size,
