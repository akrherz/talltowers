...
def create_netcdf(valid):
    ...
    for level in [5, 10, 20, 40, 80, 120]:
        for vname, unit in zip(
            ["diag", "ts", "u", "v", "w"],
            ["unitless", "C", "m/s", "m/s", "m/s"],
        ):
            vn = "%s_%sm" % (vname, level)
            v = nc.createVariable(
                vn, np.double, ("time", "sample", "station"), fill_value=1e37
            )
    ...
    for vname in [
        ...
        v = nc.createVariable(
            vname, np.double, ("time", "station"), fill_value=1e37
        )
        units = "m/s"
...
def write_sonic_data(valid, nc):
    ...
    df = read_sql(
        """
        SELECT * from data_sonic_"""
        + table
        + """
        where valid >= %s and valid < %s and tower in (0, 1)
    """,
        PGCONN,
        params=(valid, valid + datetime.timedelta(hours=4)),
        index_col=None,
    )
...
def write_analog_data(valid, nc):
    ...
    for col in tqdm(df.columns):
        if col in ["tower", "valid", "delta"]:
            continue
        data = (
            np.ones(nc.variables[col.replace("nwht", "nw")].shape, np.double)
            * 1e37
        )
